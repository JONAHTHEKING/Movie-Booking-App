...
function Bookingpage() {
    ...
    async function fetchBooking() {
      try {
        const response = await fetchWithToken(
          "http://127.0.0.1:8000/api/user/booking/?movie=" +
            movie_id +
            "&theater=" +
            theater_id
        );
  ...
  useEffect(() => {
    async function fetchMovie() {
      try {
        const response = await fetchWithToken(
          "http://127.0.0.1:8000/api/booking/movie/?movie=" + movie_id
        );
        if (response.ok) {
          const result = await response.json();
          setMovie(result);
      ...
      >
        ...
        >
          {movie && theater && t[0] && (
            ...
            >
              <h2 style={{ color: "white", textShadow: "2px 2px 5px black" }}>
                Movie: {m[0]["title"]}
              </h2>
              <h2 style={{ color: "white", textShadow: "2px 2px 5px black" }}>
                Theater: {t[0]["name"]}
          ...
          <p style={{ color: "white", textAlign: "center", marginTop: "20px" }}>
            ...
            {userbooking && userbooking.length > 0
              ? userbooking[0].total_cost
              : 0}
          </p>

          <button onClick={() => window.location.href = "/user/bookings/"} className="send-button">
    Confirm Book
</button>
